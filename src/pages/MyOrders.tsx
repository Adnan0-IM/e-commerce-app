...
const MyOrders: React.FC = () => {
  ...
  useEffect(() => {
    const fetchUserOrders = () => {
      try {
        ...
        // Get all orders from localStorage first
        const savedOrders = localStorage.getItem("orders");
        const allOrders: Order[] = savedOrders ? JSON.parse(savedOrders) : [];

        // If there are no orders at all, just show empty state
        if (!allOrders.length) {
          setOrders([]);
          setLoading(false);
          return;
        ...
        if (!userEmail) {
          // Show all orders if no email specified (for demo purposes)
  ...
  const getStatusBadgeClass = (status: OrderStatus | string): string => {
    switch (status) {
      case "pending":
                  ...
                  <div className="mt-6 border-t border-gray-200 dark:border-gray-700 pt-4">
                    ...
                    <div className="mb-6">
                      ...
                      <div className="flex items-center justify-between w-full max-w-3xl mx-auto">
                        {/* Active status depends on current order status */}
                        <div
